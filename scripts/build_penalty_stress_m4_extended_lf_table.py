...
def one_match(root: Path, scale: str) -> Path:
    pattern = (
        ...
        f"lf_scale_{scale}_l_ell_scale_0p001_m4_evalm4_lfestlegacy_axis_quantile_*_"
...
def main() -> None:
    ...
    bold = {
        "0p5": set(),
        "1": {"false_certification_rate", "mean_feasible_ratio"},
        "2": {"false_certification_rate"},
        "4": {"svr", "d_svr", "false_certification_rate"},
        "8": {"rmse", "safety_iou", "dead_end_iou", "false_certification_rate"},
    }
    lines = [
    ...
    for file_scale, display_scale, ddof in scales:
        ...
        cells = []
        for _, key in METRICS:
            cell = f"{summary[key][0]:.3f} ± {summary[key][1]:.3f}"
