...
workers="${WORKERS:-12}"
output_dir="${OUTPUT_DIR:-result/penalty_stress_m1_lf_sweep}"
...
"${python_bin}" main.py \
  ...
  --fa-beta-multiplier 0.25 \
  --fa-lf-estimator jacobian \
  --fa-lf-quantile 0.5 \
  --fa-lf-scale-sweep 0.5 1.0 2.0 \
  --fa-l-ell-quantile 0.9 \
  --fa-l-ell-scale-sweep 0.14 \
  --paired-seeds \
  --parallel-workers "${workers}" \
  --max-tasks-per-child 1 \
  2>&1 | tee "${log_path}"

"${python_bin}" scripts/build_penalty_stress_m1_lf_table.py \
  --result-dir "${output_dir}" \
  --output "${table_path}"
