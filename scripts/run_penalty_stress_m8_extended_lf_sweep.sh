...
workers="${WORKERS:-12}"
output_dir="${OUTPUT_DIR:-result/penalty_stress_m8_extended_lf_sweep}"
...
"${python_bin}" main.py \
  ...
  --fa-lf-quantile 0.5 \
  --fa-lf-scale-sweep 0.5 1.0 2.0 4.0 8.0 \
...
"${python_bin}" scripts/build_penalty_stress_m8_extended_lf_table.py \
  --result-dir "${output_dir}" \
  --output "${table_path}"
