...
def summarize(records: list[dict], fa_sal: bool = False) -> dict[str, tuple[float, float]]:
    summary = {}
    for _, key, _, _ in METRICS:
        values = np.asarray([float(record[key]) for record in records], dtype=float)
        ddof = 0 if fa_sal and key != "runtime_seconds" else 1
        summary[key] = (float(values.mean()), float(values.std(ddof=ddof)))
    return summary
...
def main() -> None:
    ...
    patterns = (
        ("FA-SAL (UM)", True, "penalty_stress_fa_sal_policy_uncertainty_max_lf_scale_1_l_ell_scale_0p14_m1_evalm1_lfestjacobian_*_rounds150_trials10_results.json"),
        ("FA-SAL (RS)", True, "penalty_stress_fa_sal_policy_random_safe_lf_scale_1_l_ell_scale_0p14_m1_evalm1_lfestjacobian_*_rounds150_trials10_results.json"),
        ("FA-SAL (GM)", True, "penalty_stress_fa_sal_policy_greedy_margin_lf_scale_1_l_ell_scale_0p14_m1_evalm1_lfestjacobian_*_rounds150_trials10_results.json"),
        ("SAL-NX", False, "penalty_stress_salnx_alpha_0p01_m1_evalm1_lfestjacobian_*_rounds150_trials10_results.json"),
        ("Tebbe-ABM", False, "penalty_stress_tebbe_abm_alpha_0p01_confidence_delta_0p01_m1_evalm1_ucand11_mcprefix0_localref0_lfestjacobian_*_rounds150_trials10_results.json"),
    )
    summaries = [(method, summarize(load_records(one_match(args.result_dir, pattern)), fa_sal)) for method, fa_sal, pattern in patterns]
